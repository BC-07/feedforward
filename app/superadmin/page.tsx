...
function toTimestampOrZero(value?: string) {
  if (!value) return 0;
  ...
  */
  ...
  const filteredCategoryControlRows = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      const createdDiff = toTimestampOrZero(a.createdAt) - toTimestampOrZero(b.createdAt);
  ...
  const latestCategoryUpdate = manageableCategories.reduce<number>(
    (latest, category) => Math.max(latest, toTimestampOrZero(category.updatedAt)),
    0,
  );
                      ...
                      {filteredCategoryControlRows.length === 0 ? (
                        ...
                        paginatedCategoryControlRows.map((row) => (
                          ...
                          >
                            ...
                            <TableCell className="py-2 align-top">
                              {new Date(toTimestampOrZero(row.createdAt)).toLocaleDateString(
                                "en-US",
                                {
                                  month: "short",
                                  day: "numeric",
