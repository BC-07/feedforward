...
export default function AdminDashboard() {
  ...
  useEffect(() => {
	...
	const admin = {
	  ...
	  unit:
		...
		localStorage.getItem("currentAdminSchool") ||
		"",
	};
  ...
  const loadFeedbacks = (unit: string) => {
	if (typeof window === "undefined") return;
	const stored: Feedback[] = JSON.parse(localStorage.getItem("feedbacks") || "[]");
  ...
  const handleUpdateFeedback = () => {
	if (!selectedFeedback || typeof window === "undefined") return;

	const allFeedbacks: Feedback[] = JSON.parse(localStorage.getItem("feedbacks") || "[]");
  ...
  const handleUnitChange = () => {
	if (!newUnit || newUnit === currentAdmin?.unit || typeof window === "undefined") return;

	const admins: AdminRecord[] = JSON.parse(localStorage.getItem("admins") || "[]");
	const unitTaken = admins.some(
	  (a) => (a.unit === newUnit || a.department === newUnit) && a.id !== currentAdmin?.id
	);
	if (unitTaken) {
	...
	const updatedAdmins = admins.map((a) =>
	  a.id === currentAdmin?.id ? { ...a, unit: newUnit, department: newUnit } : a
	);
  ...
  const filteredFeedbacks = feedbacks
	.filter((feedback) => {
	  ...
	  const matchesStatus =
		filterStatus === "all" ||
		feedback.status.toLowerCase().replace(" ", "") === filterStatus;
	  const matchesPriority =
		filterPriority === "all" || feedback.priority?.toLowerCase() === filterPriority;
							  ...
							  {selectedFeedback && (
								<Tabs defaultValue="details" className="w-full">
								  ...
								  <TabsContent value="manage" className="space-y-4">
									...
									<div className="space-y-2">
									  <Label htmlFor="response">Add/Update Response</Label>
									  <Textarea
										id="response"
		  ...
		  <div className="mt-8 space-y-6">
			<div className="flex flex-col items-center gap-3 pb-6 border-b">
			  ...
			  <div className="text-center">
				...
				<p className="text-sm text-muted-foreground">{currentAdmin?.unit}</p>
			  </div>
			</div>
			...
			<div className="space-y-3 pt-4 border-t">
			  <div>
				...
				<p className="text-xs text-muted-foreground mb-3">
				  Each unit can only have one admin. If the selected unit is already taken, the
			  ...
			  <Select value={newUnit} onValueChange={setNewUnit}>
				...
				<SelectContent>
				  ...
				  <SelectItem value="Finance & Registrar Sector">
					Finance &amp; Registrar Sector
				  </SelectItem>
				  <SelectItem value="Student Affair Sector">Student Affair Sector</SelectItem>
				  <SelectItem value="Guidance Sector">Guidance Sector</SelectItem>
				  <SelectItem value="Faculty Sector">Faculty Sector</SelectItem>
				</SelectContent>
			  </Select>
			  <Button
				className="w-full bg-accent hover:bg-accent/90"
